...
NlLen_map = {"Time": 3900, "Math": 3000, "Lang": 350, "Chart": 2350, "Mockito": 1370, "Codec": 500, "Compress": 1000,
             "Gson": 1500, "Cli": 1000, "Jsoup": 2000, "Csv": 1000, "JacksonCore": 1000, 'JacksonXml': 500,
             'Collections': 500}
CodeLen_map = {"Time": 1300, "Math": 1000, "Lang": 350, "Chart": 5250, "Mockito": 280, "Codec": 500, "Compress": 1500,
               "Gson": 1500, "Cli": 1000, "Jsoup": 2000, "Csv": 1000, "JacksonCore": 1000, 'JacksonXml': 500,
               'Collections': 500}
                ...
                for k, devBatch in tqdm(enumerate(val_set.Get_Train(len(val_set)))):
                    ...
                    with torch.no_grad():
                        ...
                        l, pre, _ = model(devBatch[0], devBatch[1], devBatch[2], devBatch[3], devBatch[4], devBatch[5],
                                          devBatch[6], devBatch[7], devBatch[8], devBatch[9])
...
if __name__ == "__main__":
    ...
    open('%sres%d_%d_%s_%s.pkl' % (p, int(sys.argv[1]), args.seed, args.lr, args.batch_size), 'wb').write(
        pickle.dumps(res))
